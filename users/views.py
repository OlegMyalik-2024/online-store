...
def login(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            )  # Сессия неавторизованного пользователя
            if user:
        ...
        form = UserLoginForm()
    context = {"title": "HelloMobile - Авторизация", "form": form}
...
def registration(request):
    ...
    context = {"title": "HelloMobile - Регистрация", "form": form}
    return render(request, "users/registration.html", context)


# Обработка личного кабинета пользователя
@login_required
def profile(request):
    # Обработка формы
    if request.method == "POST":
